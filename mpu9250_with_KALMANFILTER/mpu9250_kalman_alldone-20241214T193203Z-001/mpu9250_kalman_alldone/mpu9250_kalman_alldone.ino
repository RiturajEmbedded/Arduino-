...
delay(1000);
  ...
  delay(1000);
  float rollAcc = atan2(gValue.y, gValue.z) * RAD_TO_DEG;
  float pitchAcc = atan(-gValue.x / sqrt(gValue.y * gValue.y + gValue.z * gValue.z)) * RAD_TO_DEG;
  ...
  float rollGyro = gyr.x * dt;
  float pitchGyro = gyr.y * dt;
  float yawGyro = gyr.z * dt;

  // Apply Kalman filter
  float roll = kalmanX.getAngle(rollAcc, rollGyro, dt);
  float pitch = kalmanY.getAngle(pitchAcc, pitchGyro, dt);
  float yaw = kalmanZ.getAngle(0, yawGyro, dt);  // Assuming initial yaw angle is zero
